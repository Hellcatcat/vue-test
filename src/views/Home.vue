...
export default {
    ...
    mounted() {
        getData().then(({ data }) => {
            ...
            echartslOption.xAxis = xAxisData
            echartslOption.yAxis = {}
            echartslOption.legend = xAxisData
            echartslOption.series = []
            xAxis.forEach(key => {
                echartslOption.series.push({
                    name: key,
                    data: orderData.data.map(item => item[key]),
                    type: 'line'
                })
            })
            console.log(echartslOption);
            //使用刚指定的配置项和数据显示图表
            echarts1.setOption(echartslOption)
...
.num {
    display: flex;
    flex-wrap: wrap;
    justify-content: space-between; //所有元素贴边
